...
import { Await, FetcherWithComponents, Link, useFetchers, useNavigation } from '@remix-run/react'
...
export function Layout({
  ...
  return (
    <>
      ...
      <main>{children}</main>
      {!location.pathname.startsWith('/account') && location.pathname !== '/filters' && (
        <Suspense>
...
function CartAside({ cart, pocketItems }: any) {
  ...
  return (
    <>
      {isModalOpen && (
        <div className='dialog-overlay' onClick={() => toggleModal(false)}>
      ...
      {isPocketOpen && (
        <div className='pocket-items' onClick={handleOutsideClickPocket}>
          <div className='pocket-dialog'>
            <div className='pocket-items-header'>
              <img src='/cart/pocketItems.png' alt='pocketItems' />
              <p>Ajouter quelques items pour atteindre la couronne supérieure.</p>
            </div>
              ...
              >
                {pocket?.map((item: any, index: number) => (
                  <SwiperSlide
                    ...
                    className={`pocket-items-product ${item.variants.nodes[0].availableForSale ? '' : 'pocketItemOOO'}`}
                  >
                    <div
                    ...
                    >
                      {!item.variants.nodes[0].availableForSale && <span>Sold out</span>}{' '}
                      <img
                        src={item.images.nodes[0].url}
                        className='pocket-items-product-img-main'
                        style={{
                          objectFit: item?.box?.value == '4:5' || item?.box?.value == '1:1' ? 'contain' : 'cover',
                        }}
                      />
                      <img src={'/cart/pocket/' + (index % 8) + '.png'} className='pocket-items-product-img-price' />
                      <p>+{item.variants.nodes[0].price.amount.split('.')[0]}€</p>
                    </div>
                    <div
                      onClick={() => {
                        window.location.href = `/products/${item.handle}`
                    ...
                    >
                      <h4 className='p-title'>{item.vendor}</h4>
                      <h6 className='p-title'>{item.title}</h6>
                    </div>
                    <AddToCartButton
                      disabled={!item.variants.nodes[0].availableForSale}
                      lines={[
                        {
      ...
      {(fetcher: FetcherWithComponents<any>) => (
        <>
          ...
          <button type='submit' onClick={onClick} disabled={disabled ?? fetcher.state !== 'idle'}>
            {fetcher.state !== 'idle' ? 'Ajout ...' : 'Ajouter au panier'}
          </button>
        </>
      )}
    </CartForm>
          ...
          {({ fetchResults }) => (
            <div>
              ...
              <button
                ...
                className={inputRef.current?.value?.length === 0 ? 'btn-disabled' : 'btn-active'}
              >
                <svg xmlns='http://www.w3.org/2000/svg' width='21.798' height='21.797' viewBox='0 0 21.798 21.797'>
                  <g id='icon' transform='translate(0 0)'>
                    <path
                      ...
                      fill={inputRef.current?.value?.length !== 0 ? '#000' : '#fff'}
                    />
                    <path
                      id='Tracé_220'
                      data-name='Tracé 220'
...
function ToggleModal(toggle: any, { cart }: any) {
  return (
    ...
    >
      ...
      <p>
        Nous avons crée trois catégories d’achats pour nuancer les différentes options de retours et remboursement.{' '}
        <br />
        Repérez-les lors de vos achats pour comprendre les modalités de renvois/livraison et choisir ce qui vous
        convient le mieux.
      </p>
      <div
