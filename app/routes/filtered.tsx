...
export async function loader({ context, request }: LoaderFunctionArgs) {
  ...
  function formatCollection(titles: any) {
    ...
    if (titles?.length === 0) {
      ...
      titleString = titles.map((title: any) => `title:${title}`).join(' OR ')
    }
...
export default function filtered() {
  const { products, allFitlers, allProducts, randomProduct, metafieldRandom } = useLoaderData<typeof loader>()
    ...
    <>
      <div
        ...
        style={{
          ...
          justifyContent: productList.nodes?.length === 0 ? 'center' : '',
        }}
      >
            ...
            {isRandom ? (
              width > 768 ? (
                ...
                <div className='responsive-products-grid'>
                  {randomProducts.map((product: any, index: number) => {
                    return <MainProduct key={index} product={product} isDouble={true} />
                  })}
                </div>
              )
            ) : (
                  ...
                  <>
                    ...
                    {width > 768 ? (
                      <div className='panel-products-grid'>
                        <div className='panel-products-grid'>
                          {nodes.map((product) => (
                            <MainProduct product={product} quantity={productList?.nodes?.length} />
                          ))}
                        </div>
                      </div>
                    ) : (
                      <div className='responsive-products-grid'>
                        {nodes.map((product) => (
                          <MainProduct product={product} isDouble={true} quantity={productList?.nodes?.length} />
                        ))}
                      </div>
                    )}
