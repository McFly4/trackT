...
export default function HomePage() {
  ...
  const { products, fastShip, exclusive, hero } = useLoaderData<typeof loader>()
  ...
  const onAutoplayTimeLeft = (s: any, time: any, progress: any, index: number) => {
    ...
    >
      <div className='home'>
        <Swiper
          ...
          onAutoplayTimeLeft={(s, time, progress) => onAutoplayTimeLeft(s, time, progress, 0)}
        >
                ...
                >
                    ...
                    <img
                      src={
                        hero.fields?.find(
                          (field: any) =>
                            field?.key ===
                            (sizeScreen > 1600 ? 'fileBig' : sizeScreen > 768 ? 'file_desktop' : 'file_mobile')
                        )?.reference?.image?.url
                      }
                      alt='home'
                      style={{
                  ...
                  {sizeScreen > 768 && (
                    <div
                      className='shop-now'
                      style={{
                        backgroundColor: hero?.fields?.find((field: any) => field?.key === 'color_txt')?.value,
                      }}
                    >
                      <Link to={hero.fields?.find((field: any) => field?.key === 'link')?.value}>
                        <h4
                          style={{
                            color: hero.fields?.find((field: any) => field?.key === 'color')?.value,
                          }}
                        >
                          {hero.fields?.find((field: any) => field?.key === 'bouton')?.value}
                        </h4>
                      </Link>
                    </div>
                  )}
              ...
              <>
                ...
                <div className='header' id='shop'>
                  ...
                  <p>
                    ...
                    Panel Trackt est une nouvelle aventure. Explorez, découvrez, et laissez-vous inspirer. Bienvenue
                    dans l&apos;avenir du shopping streetwear!
                  </p>
                </div>
              </>
            )}
            {sizeScreen < 768 && (
              <div className='responsive-home-header'>
                ...
                <p>
                  Découvrez le Panel Trackt, une sélection soigneusement élaborée de ce que le streetwear a de mieux à
                  offrir. Ici, nous rassemblons des pièces uniques, des tendances émergentes et des collaborations
                  exclusives, tout en un seul endroit pour enrichir votre expérience de shopping. Chaque visite au Panel
                  Trackt est une nouvelle aventure. Explorez, découvrez, et laissez-vous inspirer. Bienvenue dans
            ...
            {sizeScreen > 768 && (
              <div className='panel-products-grid'>
                {productsList.map((product: any) => (
                  <MainProduct product={product} quantity={productsList?.length} />
                ))}
              </div>
            )}
          </div>
      ...
      {sizeScreen < 768 && <TrackT products={productsList} link={'/filtered'} />}
      ...
      <div className='hfooter'>
        <div className='panel-container-box'>
          <div className='panel-container-box-item'>
            ...
            {sizeScreen > 768 ? (
              <p>
                Entrez dans l’univers Trackt, où chaque interaction est une expérience unique. Notre équipe dédiée au
                support client est le cœur battant de notre atelier mystérieux. Nous sommes là pour vous guider, vous
                inspirer et répondre à vos questions avec une touche personnelle. <br /> <br />
                Que vous cherchiez des conseils sur la dernière tendance ou des détails sur une pièce rare, notre
                équipe, en coulisses, travaille avec passion pour vous offrir une assistance sur-mesure. C’est le
                conseil d’un connaisseur, le soutien d’un ami, le secret d’un atelier où chaque question trouve sa
                réponse.
              </p>
            ) : (
              <div className='responsive-show-more'>
                {!showText1 ? (
                  <p>Ici chaque interaction est une expérience unique. Notre équipe dédiée au support client est le…</p>
                ) : (
                  <p>
                    Entrez dans l’univers Trackt, où chaque interaction est une expérience unique. Notre équipe dédiée
                    au support client est le cœur battant de notre atelier mystérieux. Nous sommes là pour vous guider,
                    vous inspirer et répondre à vos questions avec une touche personnelle. <br /> <br />
                    Que vous cherchiez des conseils sur la dernière tendance ou des détails sur une pièce rare, notre
                    équipe, en coulisses, travaille avec passion pour vous offrir une assistance sur-mesure. C’est le
                    conseil d’un connaisseur, le soutien d’un ami, le secret d’un atelier où chaque question trouve sa
                    réponse.
                ...
                <button onClick={() => setShowText1(!showText1)}>{showText1 ? '-' : '+'}</button>
              </div>
            )}
          </div>
          <div className='panel-container-box-item'>
            ...
            {sizeScreen > 768 ? (
              <p>
                Ici chaque interaction est une expérience unique. Notre équipe dédiée au support client est le cœur
                battant de notre atelier. Nous sommes là pour vous guider, vous inspirer et répondre à vos questions
                avec une touche personnelle. Que vous cherchiez des conseils sur la dernière tendance ou des détails sur
                une pièce rare, notre équipe, en coulisses, travaille avec passion pour vous offrir une assistance
              ...
              <div className='responsive-show-more'>
                {!showText2 ? (
                  ...
                  <p>
                    Ici chaque interaction est une expérience unique. Notre équipe dédiée au support client est le… cœur
                    battant de notre atelier. Nous sommes là pour vous guider, vous inspirer et répondre à vos questions
                    avec une touche personnelle. Que vous cherchiez des conseils sur la dernière tendance ou des détails
                    sur une pièce rare, notre équipe, en coulisses, travaille avec passion pour vous offrir une
                    assistance sur-mesure. Disponible 7 / 7
                  </p>
                )}

                <button onClick={() => setShowText2(!showText2)}>{showText2 ? '-' : '+'}</button>
              </div>
            )}
          </div>
        </div>
        <TrackT products={fastShipProducts} title='Chez vous en 48h' link={'/filtered?fast=true'} />
        <MarketDrag />
        <GoFilters />
        <TrackT products={exclusiveProducts} title='Exclusive Items' link={'/filtered?release=true'} />
      </div>
    </div>
  )
}
